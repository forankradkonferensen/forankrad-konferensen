...
const Banner: React.FC<BannerTypes> = ({ title, text, bibleRef, buttonText, buttonHref, image }) => {
    ...
    <div className="absolute inset-0 flex flex-col justify-center items-center text-center z-10">
        ...
        <h4 className="text-lg md:text-2xl text-shadow-xl p-2 font-medium">{text}</h4>
        <p className="text-sm md:text-base">{bibleRef}</p>
        {buttonHref && buttonText && (
            <a className="btn hover:bg-whiteShade outline-none w-32 md:w-48 text-black" href={buttonHref}>{buttonText}</a>
        )}
    </div>
    <div className="absolute bottom-0 left-0">
       {image && <Image width={1500} height={1500}  src={image} alt="" />} 
    </div>
</div>
